...
Let's get started on the variables we can randomise first - A, B, and C. There are a few restrictions on our randomisation: 
...
$A = mt_rand(25, 40);
$B = mt_rand(10, $A - 5);       // B always smaller than A
...
As you can see, the maximum range of the variable `$B` is decided by `$A` to ensure it is always smaller. Meanwhile, instead of randomly generating C (or rather, `$spentA` in this case), I've instead generated `$kilos_bought` as its own variable, so that `$spentA` is always some multiple of `$A`. It also allows us to calculate the answer very clearly. 
...
Next up, let's begin by defining the box. 

```js
let box = JXG.JSXGraph.initBoard('graph', {boundingbox: [-1, A*11, 11, -A], showNavigation: false, axis: true});
```
...
- The `boundingbox` decides the top-left `(-1, A*11)` and bottom-right `(11, -A)` coordinates of the axes; I've made it depend on `A` so that it shows all the values.
...
<script>
    let box = JXG.JSXGraph.initBoard('ex1', {boundingbox: [-1, 330, 11, -30], showNavigation: false, axis: true});
</script>

Now we just need to add in the lines (and the appropriate labels) - we can use the `.create()` method on the `box` we just created: 

```js
let lineA = box.create('functiongraph', [(n) => {return A*n}, 0, 11], {name: "Farm A", withLabel: true, label: {position: 'mid', offset: [30, 30]}, color: 'black', strokeWidth: 2});
let lineB = box.create('functiongraph', [(n) => {return B*n}, 0, 11], {name: "Farm B", withLabel: true, label: {position: 'mid', offset: [-30, -30]}, color: 'blue', strokeWidth: 2});
```
...
- The second argument is an array of 3 values `[(n) => {return A*n}, 0, 11]`. 
  - The first value, `(n) => {return A*n}`, is a function - in this case, it's the line $$y = Ax$$
  - The second and third values, 0 and 11, specify the range of values that this function will be run on
- The third argument includes our other generic arguments that modify how our function looks: 
  ...
  - The location of the label is set through the `label` argument itself
...
let lineA = box2.create('functiongraph', [(n) => {return 30*n}, 0, 11], {name: "Farm A", withLabel: true, label: {position: 'mid', offset: [30, 30]}, color: 'black', strokeWidth: 2});
let lineB = box2.create('functiongraph', [(n) => {return 20*n}, 0, 11], {name: "Farm B", withLabel: true, label: {position: 'mid', offset: [-30, -30]}, color: 'blue', strokeWidth: 2});
</script>
...
> -- Incomplete Section
> 
> There's actually an issue here where the automatically-generated axes doesn't draw enough lines properly; consider: 
> - Add showNavigation back in
> - Generate lines from 0 to 20 or 100 or however big
> - Find a way of adding more axes lines
> - Provide explanations of graph interaction (hold shift and drag/scroll to pan/zoom)